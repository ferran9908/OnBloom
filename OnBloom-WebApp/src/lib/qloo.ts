...
export class QlooApiService {
  ...
  constructor(config: QlooConfig) {
    ...
    const defaultBaseUrl = "https://hackathon.api.qloo.com";
  ...
  async getInsights(params: InsightsParams): Promise<InsightsResponse> {
    try {
      ...
      if (params.signal?.demographics?.gender) {
        queryParams["signal.demographics.gender"] =
          params.signal.demographics.gender;
      }
      if (params.signal?.demographics?.audiences) {
        queryParams["signal.demographics.audiences"] =
          params.signal.demographics.audiences;
      }

      // Add interest signals
      if (params.signal?.interests?.entities) {
        queryParams["signal.interests.entities"] =
      ...
      if (params.filter?.geocode?.country_code) {
        queryParams["filter.geocode.country_code"] =
  ...
  async searchAudiences(
    params: AudienceSearchParams = {},
  ): Promise<{ results: QlooAudience[] }> {
    try {
      const queryParams: any = {};
...
export class QlooUtils {
   ...
   */
  static createDateRange(daysBack: number = 30): {
    startDate: string;
    endDate: string;
  } {
...
export function createQlooService(
  apiKey: string,
  options?: Partial<QlooConfig>,
): QlooApiService {
...
export const AgeGroups = {
  YOUNG_ADULT: "35_and_younger" as AgeGroup,
  MIDDLE_AGE: "36_to_55" as AgeGroup,
  SENIOR: "55_and_older" as AgeGroup,
  AGE_24_AND_YOUNGER: "24_and_younger" as AgeGroup,
  AGE_25_TO_29: "25_to_29" as AgeGroup,
  AGE_30_TO_34: "30_to_34" as AgeGroup,
  AGE_35_TO_44: "35_to_44" as AgeGroup,
  AGE_45_TO_54: "45_to_54" as AgeGroup,
} as const;
